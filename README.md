...
| Área                    | Funcionalidad                                                                                                                                |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 📋 **Expedientes**      | Gestión de casos disciplinarios con **24 estados** organizados en 5 fases: Recepción, Investigación, Resolución, Apelación y Seguimiento.    |
...
- La suite actual valida **769 pruebas unitarias en 171 suites**, E2E de navegación/privacidad, auditoría axe pública y auditoría de dependencias sin vulnerabilidades.
...
El 2026-08-16 se aplicaron con `supabase db push --linked --include-all` las migraciones `20260815170000_harden_convivencia_rls_roles.sql` (restricción de rol en RLS de las tablas de expediente: UPDATE/DELETE de `disciplinary_processes` y DELETE de bitácora/checklist/cartas/etapas/análisis solo para `admin`/`direccion`/`superadmin`, escritura de `disciplinary_rules` solo administrativa) y `20260815173000_fix_generate_process_number_tenant.sql` (`generate_process_number` ahora rechaza `p_tenant_id` ajeno para usuarios autenticados con `42501 tenant mismatch`). Ambas son forward-only sobre tablas CONVIVENCIA; no se tocaron objetos de inasistencias.
