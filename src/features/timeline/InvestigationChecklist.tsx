...
export default function InvestigationChecklist({
  ...
  const derivationItem =
    model.mediationFlowItems.find((item) => item.id === "chk_inv_3") ?? null;
  const renderInvestigationItem = (
    item: ChecklistItem,
    notRequired = false,
  ) => (
    <div key={item.id}>
  ...
  return (
    <div
      ...
      className={`overflow-hidden rounded-lg border bg-white transition-colors ${
        isActive
          ? "border-brand-300 bg-brand-50/5 ring-1 ring-brand-300/30"
          : "border-neutral-200"
      }`}
    >
      <button
        ...
        >
          ...
          >
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h4
                  id="mediation-subflow-title"
                  className="font-semibold text-neutral-900 text-xs"
                >
                  Mediación
                </h4>
                <p className="mt-0.5 text-neutral-600 text-10px leading-snug">
                  Instancia opcional para aquellos casos en que corresponda una
                  estrategia restaurativa o colaborativa.
                </p>
              </div>
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  {canRegister && (
                    <button
                      ...
                      className="inline-flex w-fit items-center gap-1.5 rounded border border-brand-200 bg-white px-2.5 py-1 font-semibold text-11px text-brand-700 transition-colors hover:bg-brand-50"
                    >
