  ...
  if (!text) {
    return (
      <p className="text-neutral-500 text-xs italic">
        No se ha generado contenido aún.
      </p>
    );
  }
      ...
      {lines.map((line) => {
        ...
        if (trimmed.startsWith("## ")) {
          return (
            <h3
              key={lineKey}
              className="mt-5 mb-2 flex items-center gap-2 font-bold text-base text-leve-700 text-neutral-900"
            >
              {trimmed.replace("## ", "")}
            </h3>
          );
        ...
        if (trimmed.startsWith("> ")) {
          return (
            <div
              key={lineKey}
              className="my-2 flex items-start gap-2 rounded-r-md border-grave-500 border-l-4 bg-grave-50 p-2.5 text-grave-700"
            >
