...
export default function ChecklistProgressPanel({
  ...
  const { entries, isLoading, error, createEntry, isCreating } =
    useChecklistProgress(causaId, incidenteId);
  const [isOpen, setIsOpen] = useState(false);
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [entryType, setEntryType] =
    useState<BitacoraEntry["tipo"]>("Evidencia");
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="font-semibold text-info-800 text-xs">
            Avances de este hito
          </p>
        ...
        >
          ...
          <label htmlFor={`progress-type-${item.id}`} className="space-y-1.5">
            ...
            <select
              ...
              value={entryType}
              onChange={(event) =>
                setEntryType(event.target.value as BitacoraEntry["tipo"])
              }
          ...
          <div className="space-y-1.5 sm:col-span-2">
            <label
              htmlFor={`progress-description-${item.id}`}
          ...
          <label htmlFor={`progress-date-${item.id}`} className="space-y-1.5">
            <span className="flex items-center gap-1 font-semibold text-9px text-neutral-700 uppercase">
              <CalendarPlus className="size-3.5" aria-hidden="true" /> Fecha y
              hora
            </span>
            <input
          ...
          <label
            htmlFor={`progress-document-${item.id}`}
