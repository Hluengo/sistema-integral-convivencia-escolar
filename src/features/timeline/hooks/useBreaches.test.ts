...
describe('computeBreaches', () => {
  ...
  it('alerta por resolución ejecutoriada sin informar derecho a apelación', () => {
    const causa = makeCausa({
      estadoActual: 'Resolución Ejecutoriada' as EstadoCausa,
    });
    assert.ok(
      computeBreaches(causa).some((b) => b.includes('derecho a solicitar reconsideración')),
    );
  });

  it('no alerta por apelación si chk_imp_1 está completado', () => {
    const causa = makeCausa({
      estadoActual: 'Resolución Ejecutoriada' as EstadoCausa,
      checklistDebidoProceso: makeChecklist([{ id: 'chk_imp_1', completado: true }]),
    });
    assert.ok(
      !computeBreaches(causa).some((b) => b.includes('derecho a solicitar reconsideración')),
    );
  ...
  it('alerta por medidas disciplinarias sin medida formativa previa', () => {
    const causa = makeCausa({
      estadoActual: 'En Proceso de Seguimiento' as EstadoCausa,
      medidasEjecutadas: ['Amonestación escrita', 'Suspensión 3 días'],
    });
    assert.ok(computeBreaches(causa).some((b) => b.includes('medidas formativas previas')));
  });

  it('no alerta por medidas si hay alguna formativa', () => {
    const causa = makeCausa({
      estadoActual: 'En Proceso de Seguimiento' as EstadoCausa,
      medidasEjecutadas: ['Taller de habilidades socioemocionales', 'Amonestación escrita'],
    });
    assert.ok(!computeBreaches(causa).some((b) => b.includes('medidas formativas previas')));
  });

  it('no alerta por medidas fuera de fases de ejecución', () => {
    const causa = makeCausa({
      estadoActual: 'En Proceso de Indagación' as EstadoCausa,
