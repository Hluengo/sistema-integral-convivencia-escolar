...
export default function CausasView({
  ...
  const selectedFaseFilter = useCausasStore(
    (state) => state.selectedFaseFilter,
  );
  const setSelectedFaseFilter = useCausasStore(
    (state) => state.setSelectedFaseFilter,
  );
  ...
  const courseOptions = useMemo(
    () =>
      [
        ...new Set(
          workspace.causas
            .map((causa) => causa.estudianteCurso)
            .filter(Boolean),
        ),
      ].sort((left, right) =>
        left.localeCompare(right, "es-CL", {
          numeric: true,
          sensitivity: "base",
        }),
      ),
  ...
  const visibleCausas = useMemo(
    () =>
      selectedCourse
        ? workspace.filteredCausas.filter(
            (causa) => causa.estudianteCurso === selectedCourse,
          )
        : workspace.filteredCausas,
    [workspace.filteredCausas, selectedCourse],
  );
  const visibleAulaSeguraCount = visibleCausas.filter(
    (causa) => causa.comprometeAulaSegura,
  ).length;
  const hasActiveFilters = Boolean(
    selectedCourse || searchQuery || selectedFaseFilter !== "Todas",
  );
