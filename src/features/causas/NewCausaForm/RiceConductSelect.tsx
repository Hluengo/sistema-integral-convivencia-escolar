...
export default function RiceConductSelect({
  ...
  const conductasLeves = REGLAMENTO_CONDUCTAS.filter(
    (conducta) => conducta.gravedad === "Leve",
  );
  const conductasGraves = REGLAMENTO_CONDUCTAS.filter(
    (conducta) => conducta.gravedad === "Grave",
  );
  ...
  const applyMatched = (matched: RiceConducta) => {
  ...
  const applyConducta = (conductId: string) => {
    const matched = REGLAMENTO_CONDUCTAS.find(
      (conducta) => conducta.id === conductId,
    );
    if (!matched) {
      setConductaRiceId("");
      return;
  ...
  const confirmReplacement = () => {
    ...
    const matched = REGLAMENTO_CONDUCTAS.find(
      (conducta) => conducta.id === pendingConductId,
    );
    if (!matched) return;
    applyMatched(matched);
  ...
  return (
    <div>
      ...
      >
        ...
        <optgroup
          label="Faltas Leves (Art. 24)"
          className="bg-white font-semibold text-leve-700"
        >
        ...
        <optgroup
          label="Faltas Graves (Art. 25)"
          className="bg-white font-semibold text-grave-700"
        >
      ...
      <AlertDialog
        open={pendingConductId !== null}
        onOpenChange={() => setPendingConductId(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogIcon />
          ...
          <AlertDialogDescription>
            Ya existen observaciones manuales. Si continúa, se reemplazarán por
            los antecedentes del RICE seleccionado.
          </AlertDialogDescription>
          <AlertDialogFooter>
