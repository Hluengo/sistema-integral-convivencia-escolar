    ...
    <div className="space-y-3 rounded-2xl border border-brand-200 bg-brand-50/70 p-4">
      <div>
        <h4 className="font-semibold text-brand-950">
          Comparación entre los dos PDF
        </h4>
        <p className="mt-1 text-brand-800 text-xs">
          La variación compara el contenido clasificado; al confirmar se omiten
          duplicados.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl border border-white/80 bg-white p-3">
          <p
            className="truncate font-semibold text-neutral-800 text-sm"
            title={previous.file_name}
          >
          ...
          <p className="mt-2 font-semibold text-neutral-900">
            {total(previousSummary)} en total
          </p>
          <p className="text-neutral-600 text-xs">
            {previousSummary.negativas} negativas · {previousSummary.positivas}{" "}
            positivas · {previousSummary.informativas} informativas
          </p>
        </div>

        <div className="rounded-xl border border-brand-200 bg-white p-3">
          <p
            className="truncate font-semibold text-brand-950 text-sm"
            title={currentFileName}
          >
            PDF nuevo: {currentFileName}
          </p>
          <p className="mt-1 text-brand-700 text-xs">
            Analizado {formatChileDateTime(currentAnalyzedAt)}
          </p>
          <p className="mt-2 font-semibold text-brand-950">
            {total(current)} en total
          </p>
          <p className="text-brand-800 text-xs">
            {current.negativas} negativas · {current.positivas} positivas ·{" "}
            {current.informativas} informativas
          </p>
        </div>
      </div>

      <p className="rounded-lg bg-brand-100 px-3 py-2 font-medium text-brand-900 text-sm">
        Variación del PDF nuevo: {formatVariation(totalVariation)} total (
        {formatVariation(variation.negativas)} negativas ·{" "}
        {formatVariation(variation.positivas)} positivas ·{" "}
        {formatVariation(variation.informativas)} informativas)
      </p>
    </div>
  );
}
