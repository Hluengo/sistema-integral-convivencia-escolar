...
function filterStudents(
  ...
  filtered = filtered.filter((student) =>
    matchesAnnotationFilter(student, activeFilter),
  );
  filtered = filtered.filter((student) =>
    matchesCourseFilter(student, selectedCourseId),
  );
  filtered = filtered.filter((student) =>
    matchesCartaStatusFilter(
      { cartaStatuses: cartaStatuses[student.id] },
      selectedCartaStatus,
    ),
  );
      ...
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <details ref={exportMenuRef} className="group relative shrink-0">
            <summary className="inline-flex min-h-11 w-full cursor-pointer list-none items-center justify-center gap-2 rounded-lg border border-leve-200 bg-leve-50 px-4 py-2 font-semibold text-neutral-800 text-sm transition-colors hover:bg-leve-100 focus:outline-none focus:ring-2 focus:ring-leve-500/30 sm:w-auto">
              ...
              {ANNOTATION_EXPORT_OPTIONS.map((option) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left text-neutral-700 text-sm transition-colors hover:bg-leve-50 hover:text-neutral-800 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    ...
                    <span className="rounded-full bg-neutral-100 px-2 py-0.5 font-semibold text-neutral-600 text-xs">
                      {count}
                    </span>
                  </button>
                );
        ...
        {/* Filter Tabs (dentro del panel de filtros) */}
          ...
          <table className="min-w-full">
            <thead className="border-b border-neutral-200/60 bg-neutral-50">
              <tr aria-label="Estado de carga de estudiantes">
                <th className="px-4 py-3 text-left font-semibold text-neutral-700 text-xs">
                  Estudiante
                </th>

                <th className="hidden px-4 py-3 text-left font-semibold text-neutral-700 text-xs md:table-cell">
                  Curso
                </th>

                <th className="px-4 py-3 text-center font-semibold text-neutral-700 text-xs">
                  Positivas
                </th>
                <th className="px-4 py-3 text-center font-semibold text-neutral-700 text-xs">
                  Negativas
                </th>
                <th className="hidden px-4 py-3 text-left font-semibold text-neutral-700 text-xs lg:table-cell">
                  Último Registro
                </th>
                <th className="px-4 py-3 text-left font-semibold text-neutral-700 text-xs">
                  Estado
            ...
            <tbody className="divide-y divide-neutral-100">
              {isLoading ? (
                ...
                filteredStudents.map((student, index) => {
                  ...
                  const studentLabel = privacyMode
                    ? `estudiante ${index + 1}`
                    : student.full_name;
                  ...
                  return (
                    <tr
                      key={student.id}
                      className={`transition-colors ${style.rowBg}`}
                    >
                      ...
                      <td className="whitespace-nowrap px-4 py-3 text-center text-neutral-600 text-sm">
                        <span className="inline-flex items-center justify-center rounded-full bg-leve-100 px-2.5 py-0.5 font-semibold text-neutral-800 text-xs">
                          {Number(student.positive_annotations_count) || 0}
                        </span>
                        {student.ai_analysis &&
                          student.ai_analysis.positivas > 0 && (
                            <span className="ml-1 inline-flex items-center rounded-full bg-brand-100 px-1.5 py-0.5 font-semibold text-10px text-brand-800">
                      ...
                      <td className="whitespace-nowrap px-4 py-3 text-center text-neutral-600 text-sm">
                        ...
                        {student.ai_analysis &&
                          student.ai_analysis.negativas > negativeCount && (
                            <span
                      ...
                      <td className="hidden whitespace-nowrap px-4 py-3 text-sm md:table-cell">
                        {(() => {
                          ...
                          const sorted = [...statuses].sort((a, b) => {
                            ...
                            if (a === "Procesada" && b === "Archivada")
                              return -1;
                            if (a === "Archivada" && b === "Procesada")
                              return 1;
                            return 0;
                          });
                          return (
