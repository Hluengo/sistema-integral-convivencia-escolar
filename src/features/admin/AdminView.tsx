...
export default function AdminView() {
  ...
  const membersQuery = useQuery({
    ...
    enabled:
      Boolean(tenantId) && (activeTab === "overview" || activeTab === "users"),
  });
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-neutral-500 text-xs">
                  Supabase enviará el correo de invitación y el acceso quedará
                  asociado a este establecimiento.
                </p>
              </div>
            </div>
            <form
          ...
          <section className="card overflow-hidden">
            <div className="border-neutral-200/70 border-b px-5 py-4">
              ...
              <p className="mt-1 text-neutral-500 text-xs">
                El sistema impide desactivar o degradar al último administrador
                activo.
              </p>
            </div>
          ...
          <section className="card overflow-hidden">
            <div className="border-neutral-200/70 border-b px-5 py-4">
              ...
              <p className="mt-1 text-neutral-500 text-xs">
                Reenvía invitaciones pendientes o cancela accesos que ya no
                correspondan.
              </p>
            </div>
                ...
                (membersQuery.data?.history ?? []).map((event) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-neutral-500 text-xs">
                        {event.actorEmail ?? "Usuario administrativo"} ·
                        registro {event.entity_id}
                      </p>
                    </div>
                    <time
...
function MemberRow({
  ...
  const [selectedRole, setSelectedRole] = useState<AdminMemberRole>(
    member.role,
  );
  const [enabled, setEnabled] = useState(
    member.is_active && member.membershipActive,
  );
  const changed =
    selectedRole !== member.role ||
    enabled !== (member.is_active && member.membershipActive);
  return (
