        ...
        <div className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {cs.map(c => (
                        <button key={c.n} type="button" onClick={() => setCourse(c.n)}
                          className={'p-3 rounded-xl border text-left transition-all ' + (course === c.n ? 'border-indigo-500 bg-indigo-50 ring-2 ring-indigo-200' : 'border-slate-200 hover:border-slate-300')}>
                          <p className="text-sm font-semibold text-slate-800">{c.n}</p>
                          <p className="text-xs text-slate-400">{c.c} estudiante{c.c !== 1 ? 's' : ''}</p>
                        </button>
                      ))}
                    </div>
                  </div>
          ...
          {step === 2 && (
            <div className="space-y-3">
              ...
              {filtered.length === 0 ? (
                <div className="text-center py-8 text-slate-400 text-sm">No hay estudiantes en este curso.</div>
              ) : (
                <div className="space-y-2 max-h-72 overflow-y-auto">
                  {filtered.map(s => (
                    <button key={s.id} type="button" onClick={() => setSelectedStudent(s)}
                      className={'w-full p-3 rounded-xl border text-left transition-all flex items-center justify-between ' + (selectedStudent?.id === s.id ? 'border-indigo-500 bg-indigo-50 ring-2 ring-indigo-200' : 'border-slate-200 hover:border-slate-300')}>
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold text-slate-800 truncate">{s.full_name}</p>
                        <p className="text-xs text-slate-400">{s.rut ? 'RUT: ' + s.rut + ' | ' : ''}{s.annotations_count ?? 0} anotacion{(s.annotations_count ?? 0) !== 1 ? 'es' : ''}</p>
                      </div>
          ...
          {step === 4 && (
            <div className="space-y-4">
                  ...
                  {detected.map((a: any, i: number) => (
                    <div key={a.id || a.date + a.text || i} className="flex items-start gap-2 p-2.5 rounded-lg bg-slate-50 border border-slate-100">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400">{a.registered_by} - {a.severity} ({a.type})</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {!isAnalyzing && !analysisError && detected.length === 0 && file && (
                <p className="text-sm text-slate-500 text-center py-4">Haz clic en "Subir y Analizar" para procesar el documento.</p>
              )}
            </div>
          )}
          ...
          {step === 5 && (
            <div className="space-y-4">
              <p className="text-sm text-slate-600 font-medium">Clasificaci\u00F3n de la Medida</p>
              <div className="space-y-2">
                {[
                  ...
                  <button key={opt.value} type="button" onClick={() => setClassification(opt.value)}
                    className={'w-full text-left p-4 rounded-xl border transition-all ' + (classification === opt.value ? 'border-indigo-500 bg-indigo-50 ring-2 ring-indigo-200' : 'border-slate-200 hover:border-slate-300')}>
                    <p className="font-semibold text-slate-800 text-sm">{opt.label}</p>
                    <p className="text-xs text-slate-500 mt-1">{opt.desc}</p>
                    <p className="text-xs text-slate-400 mt-1 font-mono">{opt.legal}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          {step === 6 && (
            <div className="space-y-4">
              <p className="text-sm text-slate-600 font-medium">Revisi\u00F3n Final</p>
              <div className="bg-slate-50 rounded-xl p-4 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-slate-500">Estudiante:</span><span className="font-medium text-slate-800">{selectedStudent?.full_name}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Curso:</span><span className="font-medium text-slate-800">{course}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Anotaciones:</span><span className="font-medium text-slate-800">{detected.length} detectadas</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Medida:</span><span className="font-medium text-slate-800">{classification === 'amonestacion' ? 'Amonestaci\u00F3n Escrita' : classification === 'compromiso' ? 'Carta de Compromiso' : 'Derivaci\u00F3n'}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Documento:</span><span className="font-medium text-slate-800">{file?.name || 'Ninguno'}</span></div>
              </div>
            </div>
          )}
