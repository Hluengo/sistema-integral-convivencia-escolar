    ...
    <div className="space-y-1.5 text-xs leading-relaxed font-sans text-left">
      {lines.map((line, idx) => {
        ...
        if (trimmed.startsWith('### ')) {
          return <h4 key={idx} className="font-bold text-neutral-900 border-b border-neutral-150 pb-0.5 mt-2">{trimmed.substring(4)}</h4>;
        }
        if (trimmed.startsWith('## ')) {
          return <h3 key={idx} className="font-bold text-xs text-brand-700 mt-3 flex items-center gap-1.5">{trimmed.substring(3)}</h3>;
        }
        if (trimmed.startsWith('# ')) {
          return <h2 key={idx} className="font-bold text-sm text-neutral-950 mt-4 border-l-2 border-neutral-700 pl-1">{trimmed.substring(2)}</h2>;
        }
        if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
          return (
            <div key={idx} className="flex items-start gap-1 ml-3 font-medium text-neutral-600">
        ...
        if (numMatch) {
          return (
            <div key={idx} className="flex items-start gap-1.5 ml-3 font-medium text-neutral-600">
        ...
        return <p key={idx} className="text-neutral-700 font-medium">{renderBoldText(trimmed)}</p>;
      })}
    </div>
  );
}
...
export default function AiAdvisor() {
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      // Map history excluding the very first model greet to avoid excessive history payloads
      ...
      setMessages(prev => [...prev, { role: 'model', content: `**Error al comunicar con el asistente:** ${e.message}` }]);
    } finally {
      setIsLoading(false);
    }
  };
