...
export type RouteIntent =
  | { kind: "view"; view: SidebarView; causaId?: string }
  | { kind: "login" }
  | { kind: "not-found" };
...
export function routeIntentFromPath(pathname: string): RouteIntent {
  ...
  if (normalizedPath === "/expedientes")
    return { kind: "view", view: "causas" };
  if (normalizedPath.startsWith("/expedientes/")) {
    const causaId = decodeURIComponent(
      normalizedPath.slice("/expedientes/".length),
    );
    return causaId
      ? { kind: "view", view: "causas", causaId }
      : { kind: "view", view: "causas" };
  }
