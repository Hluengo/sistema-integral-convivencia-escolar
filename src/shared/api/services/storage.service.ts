...
export const STORAGE_BUCKET = "documentos_convivencia";
...
export async function uploadDocument(
  ...
  if (!ALLOWED_DOCUMENT_EXTENSIONS.has(extension)) {
    throw new Error(
      "Formato no permitido. Use PDF, Markdown, Word o imagen JPG, PNG o WEBP.",
    );
  }
  if (file.size === 0) {
  ...
  if (error) {
    ...
    throw new Error(
      "No fue posible subir el documento al almacenamiento privado.",
    );
  }
  return filePath;
}
...
export async function listDocuments(
  ...
  const sharedDocuments = await listDocumentFolder(
    incidenteId,
    "documentos",
    "incidente",
  );
