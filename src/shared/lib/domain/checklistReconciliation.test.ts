...
describe("reconcileChecklistFromBitacora", () => {
  it("reconstruye hitos reforzados solo para expedientes versión 2", () => {
    const legacy = reconcileChecklistFromBitacora([], []);
    const current = reconcileChecklistFromBitacora([], [], 2);
  ...
  it("mantiene todos los hitos base aunque Supabase tenga solo una parte", () => {
    ...
    assert.equal(
      result.find((candidate) => candidate.id === "chk_rec_1")?.completado,
      true,
    );
