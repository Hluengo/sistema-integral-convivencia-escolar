...
import {
  ...
  type ExistingCausaPersistenceOperations,
...
describe("mergePendingCausaSave", () => {
  it("usa el entrante cuando no hay pendiente previo", () => {
    const causa = createCausaFixture();
    const incoming = {
      changes: { causa: false, bitacora: true, checklist: false },
      previousCausa: causa,
    };

    assert.deepEqual(mergePendingCausaSave(undefined, incoming), incoming);
  });

  it("une bloques y conserva la foto previa m�s antigua", () => {
    const oldSnapshot = createCausaFixture();
    const newSnapshot = { ...oldSnapshot, observaciones: "cambio intermedio" };

    const merged = mergePendingCausaSave(
      {
        changes: { causa: true, bitacora: false, checklist: false },
        previousCausa: oldSnapshot,
      },
      {
        changes: { causa: false, bitacora: true, checklist: true },
        previousCausa: newSnapshot,
      },
    );
    ...
    assert.equal(merged.previousCausa, oldSnapshot);
  });
});
