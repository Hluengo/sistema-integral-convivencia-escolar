...
let getChecklistEntryType: (itemLabel: string) => string;
...
before(async () => {
  ...
  ({ getChecklistEntryType } = await import("./useChecklistRegistration"));
});
...
describe("buildManualBitacoraEntry", () => {
  ...
  it("clasifica los hitos de entrevista como entrevista en la bitácora", () => {
    assert.equal(
      getChecklistEntryType("Entrevista Disciplinaria Realizada"),
      "Entrevista",
    );
    assert.equal(
      getChecklistEntryType("Recepción de Denuncia"),
      "Notificación",
    );
  });
});
