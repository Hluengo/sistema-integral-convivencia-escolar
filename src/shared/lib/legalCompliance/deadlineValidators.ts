...
export function verificarPlazoInvestigacion(causa: Causa): ResultadoPlazo {
  ...
  // El plazo estándar de 2 meses (60 días) es en días corridos (Ley 21809);
  // solo el de alta complejidad (10 días) se cuenta en días hábiles.
  const esEstandar = maxDias === MAX_PLAZO_INVESTIGACION_DIAS;
  const unidad = esEstandar ? "corridos" : "hábiles";
  ...
  if (diasRestantes <= 0) {
    return {
      ...
      fechaLimite,
      mensaje: fechaCierre
        ? `PLAZO VENCIDO: La investigación cerró fuera de los ${maxDias} días ${unidad}`
        : `PLAZO VENCIDO: La investigación ha excedido los ${maxDias} días ${unidad}`,
    };
  }
  ...
  if (diasRestantes <= DIAS_ALERTA_PLAZO_CRITICO) {
    return {
      ...
      diasTranscurridos,
      fechaLimite,
      mensaje: `ALERTA: Quedan solo ${diasRestantes} días ${unidad} para vencer el plazo de investigación`,
    };
  }
  ...
  return {
    ...
    diasTranscurridos,
    fechaLimite,
    mensaje: `Plazo de investigación: ${diasRestantes} días ${unidad} restantes`,
  };
}
...
export function verificarPlazoInformeConcluyente(causa: Causa): ResultadoPlazo {
  ...
  const diasTranscurridos = calcularDiasHabilesDesdeDiaSiguiente(
    fechaCierre,
    fechaEvaluacion,
  );
 ...
 */
export function verificarPlazoNotificacionSuperintendencia(
  causa: Causa,
): ResultadoPlazo {
  ...
  if (!causa.fechaNotificacionSuperintendencia) {
    ...
    const fechaLimite =
      calcularFechaLimiteNotificacionSuperintendencia(fechaReferencia);
    const hoy = nowDateOnly();
    const diasTranscurridos = calcularDiasHabilesDesdeDiaSiguiente(
      fechaReferencia,
      hoy,
    ...
    const diasRestantes =
      MAX_PLAZO_NOTIFICACION_SUPERINTENDENCIA_DIAS - diasTranscurridos;
