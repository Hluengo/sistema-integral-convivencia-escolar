...
- Cobertura base agregada para `authStore`, `uiStore`, `toastStore` y acciones/selectores síncronos de `causasStore`; la suite local queda en 368 tests / 78 suites.
...
- Skeletons lazy cerrados: `src/shared/Skeleton.tsx` centraliza fallbacks para shell, vistas administrativas/reportes/plataforma, detalle de expediente, modales y generador de cartas. `src/app/lazyFallbacks.test.ts` evita `fallback={null}` en `src/app` y `src/features`; suite local verificada en 368 tests / 78 suites.
- Cobertura >60% cerrada: `npm run test:coverage` mide 85.61% líneas / 85.59% ramas / 84.74% funciones, excluye sólo `api/index.js` por ser artefacto generado y falla con `--test-coverage-lines=60`.
