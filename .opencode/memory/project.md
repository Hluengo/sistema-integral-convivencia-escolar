...
- Cobertura base agregada para `authStore`, `uiStore`, `toastStore` y acciones/selectores síncronos de `causasStore`; la suite local queda en 358 tests / 77 suites.
- Auditoría legacy de `src/components/`: 30 archivos actuales; 26 son barrels de compatibilidad protegidos por `src/components/legacyCompatibility.test.ts`, 3 son componentes reales pendientes (`ClosedCases`, `InteractiveTimeline`, `TemplateEditor`) y 1 es el test de compatibilidad. `MetricCard`, `ErrorBoundary`, `ToastProvider` y `ShortcutsModal` viven ahora en `src/shared/ui/`; `Header` y sus subcomponentes viven en `src/widgets/header/`; `Sidebar` y `SidebarUserMenu` viven en `src/widgets/sidebar/`. Sus archivos bajo `src/components/` sólo reexportan para compatibilidad.
