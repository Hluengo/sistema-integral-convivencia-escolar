...
- Cobertura base agregada para `authStore`, `uiStore`, `toastStore` y acciones/selectores síncronos de `causasStore`; la suite local queda en 348 tests / 77 suites.
- Auditoría legacy de `src/components/`: 27 archivos actuales, 18 barrels de compatibilidad protegidos por `src/components/legacyCompatibility.test.ts`. `MetricCard`, `ErrorBoundary`, `ToastProvider` y `ShortcutsModal` viven ahora en `src/shared/ui/`; `Sidebar` y `SidebarUserMenu` viven en `src/widgets/sidebar/`. Sus archivos bajo `src/components/` sólo reexportan para compatibilidad.
