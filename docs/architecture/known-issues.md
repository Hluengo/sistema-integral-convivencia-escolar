...
| `components/` legacy duplicado                 | Media     | ⚡ Mitigado | 18 barrels protegidos por test; quedan componentes reales por mover gradualmente        |
...
- **Legacy `components/`**: 27 archivos actuales; 18 son barrels de compatibilidad y el resto son componentes reales pendientes de migración gradual
...
- La deuda técnica en `components/` está inventariada; `legacyCompatibility.test.ts` protege los barrels y evita reintroducir duplicación en `MetricCard`, `ErrorBoundary`, `ToastProvider`, `ShortcutsModal`, `Sidebar` y `SidebarUserMenu`.
