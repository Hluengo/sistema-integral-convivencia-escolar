...
# Revisión de rendimiento — auditoría integral 2026-08-01

## Resultado

- La carga diferida de vistas ya separa los módulos funcionales; la telemetría ahora espera a tiempo ocioso o hasta ocho segundos para no competir con el primer render.
- La medición Lighthouse local anterior registró aproximadamente FCP 3,6 s, LCP 5,2 s y rendimiento 0,53, con accesibilidad 0,94 y buenas prácticas 1,0. La advertencia está asociada principalmente a JavaScript no utilizado en la página pública.
- Una medición posterior no pudo completarse por `EPERM` de Chrome al limpiar temporales en Windows. El pipeline conserva la aserción de rendimiento en modo `warn` (objetivo 0,70) y las aserciones de accesibilidad/buenas prácticas en modo `error`/`warn`.

## Próxima medición

Repetir Lighthouse en CI Linux o en una máquina Windows sin procesos Chrome residuales antes de tomar decisiones de bundle. No elevar ni relajar umbrales hasta contar con una medición reproducible.
