  ...
  async (req, res) => {
    try {
      ...
      if (message.length > MAX_ADVISOR_MESSAGE_LENGTH) {
        res
          .status(400)
          .json({ error: "El mensaje supera el máximo permitido." });
        return;
      }
      ...
      if (!normalizedHistory) {
        res.status(400).json({
          error:
            "El historial de consulta no es válido o supera el máximo permitido.",
        });
        return;
      }
...
- JAMÁS recomiendes ni apliques una medida o sanción concreta (por ejemplo, días de suspensión o expulsión). Si te piden una medida, responde indicando qué antecedentes faltan para fundarla: hechos acreditados, participación individual, norma RICE, proporcionalidad y consideración de descargos.
- Cuando te describan un caso, responde como revisión: lista qué garantías están respaldadas, cuáles faltan y qué impide fundar una decisión todavía. La decisión final siempre es humana.
...
${legalSources}`;
      ...
      console.error(
        "Error en el Chat de Consultoría:",
